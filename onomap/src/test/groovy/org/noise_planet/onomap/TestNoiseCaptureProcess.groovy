...
class TestNoiseCaptureProcess extends JdbcTestCase {
  ...
  static def addTestRecord(Sql sql, String time, String location, List<Double> levels) {
    Map record = [track_uuid         : UUID.randomUUID().toString(),
                  ...
                  noise_level        : 10 * Math.log10(levels.sum({ Math.pow(10.0d, it / 10.0d) }) as Double),
  ...
  void testProcess1() {
    ...
    assertEquals(72.82d, (Double) sql.firstRow("SELECT LAEQ FROM NOISECAPTURE_AREA_PROFILE WHERE LOCAL_HOUR = 16").get("LAEQ"),
      0.01d)
    assertEquals(72.0d, (Double) sql.firstRow("SELECT LA50 FROM NOISECAPTURE_AREA_PROFILE WHERE LOCAL_HOUR = 16").get("LA50"),
      0.01d)
  ...
  void testProcessParty() {
    ...
    assertEquals(54.9d, (Double) sql.firstRow("SELECT AP.LAEQ FROM NOISECAPTURE_AREA_PROFILE AP,NOISECAPTURE_AREA A WHERE A.pk_area = ap.pk_area and LOCAL_HOUR = 10 and pk_party is null").get("LAEQ"),
      0.1d)
    assertEquals(51.8d, (Double) sql.firstRow("SELECT AP.LA50 FROM NOISECAPTURE_AREA_PROFILE AP,NOISECAPTURE_AREA A WHERE A.pk_area = ap.pk_area and LOCAL_HOUR = 10 and pk_party is null").get("LA50"),
      0.1d)
  }
