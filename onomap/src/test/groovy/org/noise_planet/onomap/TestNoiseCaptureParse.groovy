...
class TestNoiseCaptureParse extends JdbcTestCase {
  ...
  void testParse1() {
    ...
    sql.eachRow("SELECT * FROM noisecapture_track") { ResultSet row ->
      Integer idTrack = row["pk_track"] as Integer
      ...
      assertEquals(84.0d, row.getDouble("time_length"), 0.01d)
      assertEquals(72.94d, row.getDouble("noise_level"), 0.01d)
      ...
      // Check tags
      assertEquals(3, sql.firstRow("SELECT COUNT(*) cpt FROM  noisecapture_tag").get("cpt") as Integer)
  ...
  void testParseNull() {
    ...
    sql.eachRow("SELECT * FROM noisecapture_track") { ResultSet row ->
      ...
      assertEquals(11d, row.getDouble("time_length"), 0.01d)
      assertEquals(73.7d, row.getDouble("noise_level"), 0.01d)
  ...
  void testParseWidthExistingTags() {
    ...
    sql.eachRow("SELECT * FROM noisecapture_track") { ResultSet row ->
      ...
      assertEquals("L-ITE 502", row.getString("device_model"))
      assertEquals(69, row.getInt("pleasantness"))
      assertEquals(84d, row.getDouble("time_length"), 0.01d)
      assertEquals(72.94d, row.getDouble("noise_level"), 0.01d)
      assertEquals("f7ff7498-ddfd-46a3-ab17-36a96c01ba1b", row.getString("track_uuid"))
      assertEquals(new Timestamp(1465474618000), row.getTimestamp("record_utc"))
      // Check tags
